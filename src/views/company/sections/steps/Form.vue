...
/>
        ...
        <FieldArray name="features" v-slot="{ fields, push, remove }">
          <div class="border-t border-line pt-4 mt-4">
            ...
            <div v-for="(field, idx) in fields" :key="idx" class="grid md:grid-cols-2 gap-3 items-center">
              <base-input
                :id="`feature-key-${idx}`"
            ...
            <div class="flex justify-end mt-3">
              <button type="button" class="text-primary font-semibold" @click="push({ key: '', icon: '', ar: '', en: '' })">
...
const schema = yup.object().shape({
  ...
  (value) => !!(value || initialValues.image_preview)
  ),
icon: yup.mixed().test(
  "icon",
...
if (initialValues.image) {
  frm.append("image", initialValues.image);
}
if (initialValues.icon) {
  ...
  if (!route.params.id && Array.isArray(values.features)) {
    values.features.forEach((f, i) => {
      frm.append(`features[${i}][icon]`, f.icon || "");
      frm.append(`features[${i}][key]`, f.key || "");
