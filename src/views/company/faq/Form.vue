...
const breads = [
  ...
  },
  {
    ...
    path: `faq/form${route.params.id ? "/" + route.params.id : ""}`,
  },
];

async function getData() {
  try {
    const { data } = await axios.get(`/faq/${route.params.id}`);
    const result = data.data;

    featureDetails.value = {
      id: result.id,
      quetionAr: result.ar?.question || "",
      quetionEn: result.en?.question || "",
      answerAr: result.ar?.answer || "",
     answerEn: result.en?.answer || "",
    };
  } catch (err) {
