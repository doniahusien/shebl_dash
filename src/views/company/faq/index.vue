    ...
    >
      <div class="flex justify-end mb-5">
        <button
          ...
          @click="$router.push('/faq/form')"
        >
          <i class="fas fa-plus"></i>
          {{ t(`BUTTONS.add`, { name: t('LABELS.faq') }) }}
        </button>
      </div>
      ...
      <template v-else>
        ...
        <div v-else :class="!filteredFaqs.length ? 'h-full' : ''">
          <data-table
            :headers="headers"
            :items="filteredFaqs"
            :loading="loading"
...
const filteredFaqs = computed(() => {
  ...
  return items.value
    ...
    .filter((faq) =>
      keyword ? faq.question.toLowerCase().includes(keyword) : true
    );
});
